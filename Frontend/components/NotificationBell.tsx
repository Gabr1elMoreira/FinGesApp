...
import { Bell, Sparkles, X, Target, Clock, AlertTriangle, CheckCircle2, TrendingDown } from 'lucide-react';
...
type SmartAlert = {
  id: string;
  type: 'bill_due' | 'goal_near' | 'goal_over' | 'spending_high';
  title: string;
  body: string;
...
const NotificationBell: React.FC<NotificationBellProps> = ({
  selectedMonth, selectedYear, hasTransactions, user, transactions = []
}) => {
  ...
  const smartAlerts = useMemo((): SmartAlert[] => {
    ...
    upcoming.forEach(t => {
      ...
      alerts.push({
        ...
        body: `${t.description} — R$ ${t.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
    ...
    if (overdue.length > 0) {
      alerts.push({
        ...
        body: `${overdue.length} conta${overdue.length > 1 ? 's' : ''} vencida${overdue.length > 1 ? 's' : ''} e não pagas`,
        severity: 'danger',
      });
    }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {hasUnread && (
          <span className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 border-2 border-white dark:border-[#07080d] rounded-full flex items-center justify-center text-[8px] font-black text-white">
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wide transition-all ${
                  activeTab === 'alerts'
                    ...
                    : 'text-slate-400 dark:text-[#4a4f6e] hover:text-slate-700 dark:hover:text-slate-300'
                }`}
              >
              ...
              {activeTab === 'alerts' && (
                smartAlerts.length === 0 ? (
                  <div className="px-4 py-10 text-center">
                    ...
                    <p className="text-xs font-bold text-slate-500 dark:text-[#4a4f6e]">Tudo em dia!</p>
                    <p className="text-[10px] text-slate-400 dark:text-[#3d4060] mt-1">Nenhum alerta no momento</p>
                  </div>
                ) : (
              ...
              {activeTab === 'ai' && (
                parsedInsights.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                            isUnread ? 'bg-primary/[0.12] text-primary' : 'bg-slate-100 dark:bg-white/[0.05] text-slate-400 dark:text-[#4a4f6e]'
                          }`}>
                            <Target size={16} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <h4 className={`text-[10px] font-black uppercase tracking-wide truncate ${
                                isUnread ? 'text-primary dark:text-primary-light' : 'text-slate-700 dark:text-slate-300'
                              }`}>{insight.category}</h4>
                            ...
                            <p className={`text-[11px] font-semibold leading-snug line-clamp-2 ${
                              isUnread ? 'text-slate-700 dark:text-slate-200' : 'text-slate-500 dark:text-[#4a4f6e]'
                            }`}>{insight.text}</p>
                          </div>
                        </button>
                      );
                  ...
                  <div className="px-4 py-10 text-center space-y-3">
                    ...
                    <p className="text-xs font-bold text-slate-500 dark:text-[#4a4f6e]">Nenhum insight disponível</p>
                    {hasTransactions && (
                      <button
